...
import {
    followASeller, getBestPromotionPublications,
    getBuyerProfileById, getPromotionPublications,
    getPublicationsByBuyerId, getPublicationsByCategory, likeAPublication,
    putBuyerProfileImg,
    searchSeller,
    unfollowASeller
...
export default function FeedPage() {
    ...
    const addProductCart = (cartPublication: PublicationsResultType) => {
        ...
        const hasProduct: boolean = cartList.some((cartItem: PublicationsResultType) => cartItem.publicationId === cartPublication.publicationId);
        ...
        <div className={style.page__container}>
            <section className={style.lateral__bar}>
                <ul className={style.bar__list}>
                    <BarItem isScrollButton={true} hiddenInputValue={""} barImageUrl={houseFill} barImageAlt={"Início"}
                             barText={"Início"}/>
                    <BarItem formClass={"item__form"} hiddenInputValue={"promo_posts"} barImageUrl={offers}
                             barImageAlt={"Ofertas"} barText={"Ofertas"}/>
                    <BarItem count={count} hiddenInputValue={"wishlist_display"} barImageUrl={wishes}
                             barImageAlt={"Desejos"} barText={"Lista de Desejos"}/>
                    <BarItem hiddenInputValue={"log_out"} barImageUrl={logout} barImageAlt={"Métricas"}
            ...
            <main className={style.feed__container}>
                ...
                <section className={style.category__section}>
                    ...
                    <ul className={style.category__list}>
                        <CategoryItem hiddenInputValue={"best_promo_posts"} imageUrl={piggy}
                                      imageAlt={"Maiores Ofertas"} categoryText={"Maiores Ofertas"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={1} imageUrl={tree} imageAlt={"Natal"}
                                      categoryText={"Natal"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={2} imageUrl={tv}
                                      imageAlt={"Televisores"} categoryText={"Televisores"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={3} imageUrl={smartphone}
                                      imageAlt={"Smartphones"} categoryText={"Smartphones"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={4} imageUrl={joystick}
                                      imageAlt={"Jogos"} categoryText={"Jogos"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={5} imageUrl={laptop}
                                      imageAlt={"Eletrônicos"} categoryText={"Eletrônicos"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={6} imageUrl={lamp}
                                      imageAlt={"Decorações"} categoryText={"Decorações"}/>
                        <CategoryItem hiddenInputValue={"post_by_category"} value={7} imageUrl={shirt}
                                      imageAlt={"Roupas"} categoryText={"Roupas"}/>
                    </ul>
                </section>
                ...
                <div className={style.user__features}>
                    <ul className={style.feature__list}>
                        <li className={style.feature__group}>
                            <BottomBarForm isScrollButton={true} hiddenInputValue={"home"} imageUrl={houseFill}
                                           imageAlt={"Início"}/>
                        ...
                        <li className={style.feature__group}>
                            <BottomBarForm count={count} hiddenInputValue={"wishlist_display"} imageUrl={wishes}
                                           imageAlt={"Lista de Desejos"}/>
...
export async function action({request}: ActionFunctionArgs) {
    ...
    switch (_action) {
        ...
        case "promo_posts": {
            const response: GetPromotionPublicationsResponse = await getPromotionPublications(userId);
            if (response.publicationFiltered) {
                publications = response.publicationFiltered;
            } else if (response.error) {
                return {errors: response.error};
            }

            return {publicationFiltered: response.publicationFiltered};
        }
        case "best_promo_posts": {
            const response: GetBestPromotionPublicationsResponse = await getBestPromotionPublications(userId);
        ...
        case "like_post": {
            ...
            const response: LikeAPublicationResponse = await likeAPublication(sellerId, publicationId);
